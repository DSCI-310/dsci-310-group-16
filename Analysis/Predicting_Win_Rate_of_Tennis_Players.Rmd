...
suppressMessages(source(here::here("R/1_load-libraries.R")))
suppressMessages(source(here::here("R/2_load.R")))
suppressMessages(source(here::here("R/3_clean.R")))
suppressMessages(source(here::here("R/4_exploratory-analysis.R")))
suppressMessages(source(here::here("R/5_rmspe-functions.R")))
suppressMessages(source(here::here("R/6_regression.R")))
```
...
```{r }
player_career <- data.table::fread(here::here('data/exploratory-data-analysis-table.csv'))
...
exploratory_data_analysis_table <- data.table::fread(
  here::here(
    'data/exploratory-data-analysis-table.csv'
    )
  )
...
<!-- !!!!!!  ONLY INCLUDE THE FOLLOWING TEXT WHEN THE PREDICTORS.png FILE IS WORKING PROPERLY !!!!!!-->
...
```{r Player Quantitative Predictors png, fig.cap="All Predictor Relationships using ggpairs"}
# knitr::include_graphics(here::here("player_quantitative_predictors.png"))
```
...
kknn_single <- data.table::fread(
  here::here(
    'data/kknn-single-regression.csv'
    )
  )
...
lm_multiple <- data.table::fread(
  here::here(
    'data/lm-multiple-regression.csv'
    )
  )
...
kknn_multiple <- data.table::fread(
  here::here(
    'data/kknn-multiple-regression.csv'
    )
  )
...
best_model_prediction <- data.table::fread(
  here::here(
    'data/best-model-prediction.csv'
    )
  )
